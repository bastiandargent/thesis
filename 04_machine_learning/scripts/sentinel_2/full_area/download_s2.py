...
Process API Reference: https://docs.sentinel-hub.com/api/latest/reference/#tag/process
...
output_folder = os.path.join(base_dir, "02_raw_data", "sentinel_2", project_name)
...
function setup() {
  return {
    input: [
      "B02","B03","B04","B05","B06","B07","B08","B11","B12","SCL"
    ],
...
for window in seasonal_windows:
    ...
    for x in range(xmin, xmax, tile_size):
        for y in range(ymin, ymax, tile_size):
            ...
            print(f"Downloading {year} Tile {tile_counter}")
            ...
            if response.status_code == 200:
                filename = os.path.join(
                    output_folder, f"{year}_tile_{tile_counter:02d}.tif"
                )
...
metadata = os.path.join(output_folder, "metadata.txt")
...
with open(metadata, "w") as m:
    ...
    m.write(f"Bands: B02 B03 B04 B05 B06 B07 B08 B11 B12\n")
    m.write("Masked SCL classes: 0,1,3,6,8,9,10\n\n")
    for window in seasonal_windows:
